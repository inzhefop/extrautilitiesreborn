...
public class LunarPanelUpdateTickProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		...
		if (ExtrautilitiesrebirthModBlocks.LUNAR_PANEL.get() == (world.getBlockState(new BlockPos(x + 1, y, z))).getBlock()) {
			if (new Object() {
				public boolean canReceiveEnergy(LevelAccessor level, BlockPos pos) {
					AtomicBoolean _retval = new AtomicBoolean(false);
					BlockEntity _ent = level.getBlockEntity(pos);
		...
		if (ExtrautilitiesrebirthModBlocks.LUNAR_PANEL.get() == (world.getBlockState(new BlockPos(x, y, z + 1))).getBlock()) {
			if (new Object() {
				public boolean canReceiveEnergy(LevelAccessor level, BlockPos pos) {
					AtomicBoolean _retval = new AtomicBoolean(false);
					BlockEntity _ent = level.getBlockEntity(pos);
